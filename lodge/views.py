...
from .serealizers import RoomSerializers,BookingSerializer
...
class BookingCreateAPIView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Booking.objects.filter(room=room, start_time__lte=end_time, end_time__gte=start_time).exists():
            return Response({'message': 'Room not available. Please choose another time slot.'}, status=status.HTTP_400_BAD_REQUEST)
